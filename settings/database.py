...
async def update_server(server, member_list):
    ...
    ON CONFLICT DO NOTHING""", server.id, server.name, server.owner.id)


    await postgres.execute("""INSERT INTO logging (server_id, logchannel) VALUES ($1, $2)
    ON CONFLICT (server_id) DO NOTHING""", server.id, None)
...
async def update_db(guilds, member_list):
    ...
    await postgres.executemany("""INSERT INTO logging (server_id, logchannel) VALUES ($1, $2)
    ON CONFLICT (server_id) DO NOTHING""", ((server.id, None) for server in guilds))
...
async def load_settings():
    query = '''SELECT (server_id, prefix, profanities, autoroles, antiinvite, reassign) FROM servers'''
