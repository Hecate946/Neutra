...
class Email(commands.Cog):
    ...
    async def cog_check(self, ctx):
        if ctx.author.id in [708584008065351681, 782479134436753428, 664245010383896620]:
            return True
    ...
    async def send_email(self, address, email:str, pword:str, msg: str, subj:str):
        ...
        # send
        send_kws = dict(username=email, password=pword, hostname=self.HOST, port=587, start_tls=True)
        res = await aiosmtplib.send(message, **send_kws)  # type: ignore
        msg = "failed" if not re.search(r"\sOK\s", res[1]) else "succeeded"
        return res

    async def send_txt(self, 
        num: Union[str, int], carrier: str, email: str, pword: str, msg: str) -> Tuple[dict, str]:
    ...
    async def text(self, ctx, number: int, carrier, *, message):
        if carrier in self.CARRIER_MAP:
            ...
            await self.send_txt(number, carrier, email=self.email, pword=self.password, msg=message)
            await msg.edit(content="sent")
        
        else:
            await ctx.send(f"Carrier must be one of these: `{', '.join(self.CARRIER_MAP)}`")

    @commands.command()
    async def email(self, ctx, address, *, message):

        msg = await ctx.send("I'm sending it don't spam.")
        await self.send_email(address, email=self.email, pword=self.password, msg=message, subj=None)
        await msg.edit(content="sent")
