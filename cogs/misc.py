...
class Misc(commands.Cog):
    ...
    @decorators.command(description="Finds the 'best' definition of a word", aliases=['urban'])
    @checks.cooldown()
    async def define(self, ctx, *, search: commands.clean_content):
        ...
        async with ctx.channel.typing():
            try:
                url = await self.bot.get(f'https://api.urbandictionary.com/v0/define?term={search}', res_method="json")
            except Exception:
            ...
            await ctx.send_or_reply(f"📚 Definitions for **{result['word']}**```yaml\n{definition}```")
    ...
    async def print(self, ctx, *, url = None):
        ...
        if url == None and len(ctx.message.attachments) == 0:
            await ctx.send_or_reply("Usage: `{}print [user, url, or attachment]`".format(ctx.prefix))
            return
