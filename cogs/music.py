...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def search_source(self, ctx, search: str, *, loop: asyncio.BaseEventLoop = None, bot):
        ...
        embed=discord.Embed(title=f'Search results for:\n**{search}**', description="\n".join(lst), color=robot.color, timestamp=datetime.utcnow())
        embed.set_author(name=f'{ctx.author.name}', url=f'{ctx.author.avatar_url}', icon_url=f'{ctx.author.avatar_url}')
...
class Song:
    ...
    def create_embed(self):
        embed = (
            discord.Embed(title='Now playing', description=f'```prolog\n{self.source.title}\n```', color=constants.embed)
            ...
            .add_field(name='Likes', value=f'{constants.emotes["like"]} {self.source.likes}')
            .add_field(name='Dislikes', value=f'{constants.emotes["dislike"]} {self.source.dislikes}')
...
class Music(commands.Cog):
    ...
    async def _seek(self, ctx, time: int = 0):
        ...
        now = discord.FFmpegPCMAudio(song.source.stream_url, before_options=ffmpeg_options)
