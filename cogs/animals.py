...
async def read_shuffle_and_send(animal, ctx):
    ...
    embed = discord.Embed(title=f"{animal.capitalize()}!", color=ctx.bot.constants.embed)
...
class Animals(commands.Cog):
    ...
    @decorators.command(brief="Random picture of a cat", aliases=["cats","meow",])
    @checks.cooldown()
    ...
    @decorators.command(brief="Random picture of a duck", aliases=["quack","ducc","ducks"])
    @checks.cooldown()
    ...
    @decorators.command(brief="Random picture of a raccoon", aliases=["racc","raccoons"])
    @checks.cooldown()
    async def raccoon(self, ctx):
        """Random picture of a raccoon"""
        await read_shuffle_and_send('raccoon', ctx)

    @decorators.command(brief="Random picture of a dog", aliases=["dogs","bark",])
    @checks.cooldown()
    async def dog(self, ctx):
        """Random picture of a dog"""
        await read_shuffle_and_send('dog', ctx)

    @decorators.command(brief="Random picture of a squirrel", aliases=["squ","squirrels",])
    @checks.cooldown()
    async def squirrel(self, ctx):
        """Random picture of a squirrel"""
        await read_shuffle_and_send('squirrel', ctx)

    @decorators.command(brief="Random picture of a bear", aliases=["bears",])
    @checks.cooldown()
    async def bear(self, ctx):
        """Random picture of a bear"""
        await read_shuffle_and_send('bear', ctx)

    @decorators.command(brief="Random picture of a possum", aliases=["possums",])
    @checks.cooldown()
    async def possum(self, ctx):
        """Random picture of a possum"""
        await read_shuffle_and_send('possum', ctx)

    @decorators.command(brief="Random picture of an axolotl", aliases=["axolotls",])
    @checks.cooldown()
    ...
    @decorators.command(brief="Random picture of a bunny", aliases=["bunnies", 'rabbit'])
    @checks.cooldown()
    ...
    @decorators.command(brief="Random picture of a panda (may be a redpanda)", aliases=["pandas"])
    @checks.cooldown()
