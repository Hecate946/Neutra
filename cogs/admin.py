...
class Admin(commands.Cog):
    ...
    @commands.command(
        brief="Setup server muting system.",
        aliases=["setmuterole"]
    )
    @commands.guild_only()
    ...
    async def muterole(self, ctx, role: discord.Role = None):
        ...
        if role is None:
            role = await ctx.guild.create_role(
                name="Muted",
                reason="For the server muting system"
            )
        try:
            if ctx.guild.me.top_role.position < role.position:
                await msg.edit(
                    content=f"{self.bot.emote_dict['failed']} \
                        The muted role is above my highest role."
                )
                return
            if ctx.author.top_role.position < role.position:
                if ctx.author.id != ctx.guild.owner.id:
                    await msg.edit(
                        content=f"{self.bot.emote_dict['failed']} \
                            The muted role is above your highest role."
                    )
            query = '''UPDATE servers SET muterole = $1 WHERE server_id = $2'''
            await self.bot.cxn.execute(
                query, role.id, ctx.guild.id
            )
        ...
        if channels:
            return await msg.edit(
                content=f"{self.bot.emote_dict['failed']} \
                I do not have permission to edit channel\
                    {'' if len(channels) == 1 else 's'}: \
                        `{', '.join(channels)}`"
            )

        await msg.edit(
            content=f"{self.bot.emote_dict['success']} \
                Saved `{role.name}` as this server's mute role."
        )

    @commands.command(
        aliases=["setprefix"],
        brief="Set your server's custom prefix."
    )
    @commands.guild_only()
    @permissions.has_permissions(manage_guild=True)
    async def prefix(self, ctx, new: str = None):
        ...
        if new is None:
            ...
            if len(new) > 5:
                await ctx.send(
                    f"{ctx.author.mention}, that prefix is too long. \
                        The prefix must be five characters maximum."
                )
            else:
                ...
                await ctx.reply(
                    f"{self.bot.emote_dict['success']} \
                        The prefix has been set to `{new}`"
                )
    ...
    async def add(self, ctx, roles: commands.Greedy[discord.Role] = None):
        ...
        await ctx.send(
            f"{self.bot.emote_dict['success']} Updated autorole settings."
        )
    ...
    async def clear(self, ctx):
        ...
        await ctx.send(
            f"{self.bot.emote_dict['success']} Cleared all autoroles."
        )
    ...
    async def show(self, ctx):
        ...
        p = pagination.SimplePages(
            entries=[
                f"`{ctx.guild.get_role(int(x)).name}`" for x in autoroles
            ],
            per_page=20
        )
        p.embed.title = 'Autoroles in {} ({:,} total)'\
            .format(ctx.guild.name, len(autoroles))
    ...
    @commands.command(
        brief="Reassign roles on user rejoin.",
        aliases=['stickyroles']
    )
    @commands.guild_only()
    @permissions.bot_has_permissions(manage_roles=True)
    ...
    async def remove_words(self, ctx, *, words: str = None):
        ...
        if word_list == []:
            return await ctx.send(f"{self.bot.emote_dict['error']} This server has no filtered words.")
    ...
    @_filter.command(brief="Display a list of this server's filtered words.",
                     aliases=['show'])
    @permissions.has_permissions(manage_guild=True)
    ...
    async def lock(self, ctx, channel_=None, minutes_: int = None):
        ...
        if channel_ is None:
            ...
            try:
                channel_obj = discord.utils.get(
                    ctx.guild.text_channels, name=channel_)
        ...
        try:
            overwrites_everyone = channel.overwrites_for(
            ...
            try:
                ...
                return await msg.edit(content=f"{self.bot.emote_dict['failed']} Channel {channel.mention} is already locked.")
            ...
            await msg.edit(content=f"{self.bot.emote_dict['failed']} I have insufficient permission to lock channels.")
    ...
    async def unlock(self, ctx, channel: discord.TextChannel = None, surpress=False):
        ...
        try:
            ...
            overwrites_everyone = ctx.channel.overwrites_for(
                ctx.guild.default_role)
            ...
            await msg.edit(content=f"{self.bot.emote_dict['failed']} I have insufficient permission to unlock channels.")
