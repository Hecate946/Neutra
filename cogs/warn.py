...
class Warn(commands.Cog):
    ...
    async def warn(self, ctx, targets: commands.Greedy[discord.Member], *, reason: str = None):
        ...
        for target in targets:
            if target.id in self.bot.constants.owners: return await ctx.send('You cannot warn my developer.')
    ...
    @commands.command(brief="Revoke a warning from a user", aliases=['revokewarning','undowarning','undowarn'])
    @commands.guild_only()
    @permissions.has_permissions(kick_members = True)
